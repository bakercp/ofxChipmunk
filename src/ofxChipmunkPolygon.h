...
public:
	...
	Polygon(cpSpace* space, std::vector<glm::vec2>& points, float radius=0.f, float mass=1.f);
    Polygon(ShapePolygon* src, float mass=1.f);

	void setup(cpSpace* space, ofPolyline poly, float radius=0.f, float mass=1);
	void setup(cpSpace* space, std::vector<glm::vec2>& points, float radius=0.f, float mass=1);
};
